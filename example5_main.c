...
/**
 * We have to declare these here - they're not  in any header files we can include.
 * yyparse() is declared with an empty argument list
...
int main(int argc,char** argv) {
  ...
  while ( ( n=read(fileno(stdin), buf, BUFS )) >  0) {
      ...
      while( (yv=yylex()) != 0) { 
          printf(" yylex() yv=%d yylval.dval=%g\n", yv, yylval.dval);
